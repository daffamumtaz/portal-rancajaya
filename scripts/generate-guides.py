...
REFERENCE = ROOT.parent.parent / "Referensi Dokumen Operasional" / "Panduan-Pengelola-CMS.docx"
...
OPERATIONAL = {
    ...
    "sections": [
        ("Mengenal Halaman Pengelolaan", [
            ...
            table(["Jenis pengelolaan", "Koleksi"], [["Tambah, ubah, dan hapus", "Berita, galeri, UMKM, layanan administrasi, dan kontak"], ["Tambah dan ubah; tidak dapat dihapus", "APBDes per periode"], ["Ubah data yang tersedia", "Beranda, profil dan struktur organisasi, dusun, pertanian dan peternakan, pengantar layanan, serta identitas desa"]]),
            image("admin-dashboard.png", "Ilustrasi susunan menu berdasarkan konfigurasi Portal Rancajaya."),
            note("Penting", "Tombol Hapus hanya tersedia pada koleksi yang aman. Sebelum menghapus, pastikan entri memang tidak lagi diperlukan. Untuk koleksi lain, gunakan Draf atau matikan Tampilkan di Publik.", "gold"),
        ]),
        ...
        ("Profil Desa dan Dusun", [
            p("Identitas umum, data profil, struktur organisasi, dan data dusun dikelompokkan dalam tiga menu yang saling melengkapi."),
            steps("Buka Sistem — Identitas Desa > Pengaturan Umum untuk visi, misi, tujuan, sejarah, alamat, kontak, jam layanan, media sosial, peta, dan tahun APBDes aktif.", "Buka Profil Desa — Data & Organisasi > Profil Desa & Struktur Organisasi untuk sambutan, nama kepala desa, wilayah, penduduk, demografi, dan seluruh struktur organisasi.", "Buka Profil Desa — Wilayah & Dusun untuk memperbarui kepala dusun, penduduk, kepala keluarga, luas wilayah, dan deskripsi."),
        ...
        ("Struktur Organisasi", [
            p("Struktur organisasi kini berada sebagai satu daftar di dalam entri Profil Desa & Struktur Organisasi. Hubungan atasan menentukan garis susunan pada halaman Profil."),
            steps("Buka Profil Desa — Data & Organisasi > Profil Desa & Struktur Organisasi.", "Cari bagian Struktur Organisasi.", "Buka posisi yang ingin diperbarui atau tambahkan posisi bila diperlukan.", "Isi slug unik, nama, jabatan, atasan langsung, foto, urutan, deskripsi, tahun data, sumber, dan status validasi.", "Pastikan Kepala Desa tidak memiliki atasan dan setiap posisi lain mengarah ke slug atasan yang benar, lalu Terbitkan."),
            note("Perubahan struktur", "Periksa kembali garis organisasi di halaman Profil setelah menerbitkan perubahan. Jangan memakai slug yang sama untuk dua posisi.", "gold"),
        ]),
        ("Transparansi APBDes", [
            p("Satu entri APBDes mewakili satu periode lengkap, misalnya 2026 Awal atau 2025 Perubahan. Pos Pendapatan dan Belanja disimpan sebagai daftar di dalam entri tersebut."),
            steps("Pilih Transparansi — APBDes per Periode lalu klik + Periode APBDes.", "Isi Tahun Anggaran dan pilih versi Awal atau Perubahan.", "Pada bagian Pendapatan, tambahkan seluruh nama pos, jumlah, realisasi bila tersedia, urutan, deskripsi, tanggal pembaruan, dan sumber.", "Pada bagian Belanja, lakukan langkah yang sama untuk seluruh pos belanja.", "Pilih Draf jika belum siap atau Terbit bila telah diverifikasi, lalu atur Tampilkan di Publik.", "Terbitkan dan periksa grafik serta ringkasan APBDes di halaman Profil."),
            image("admin-apbdes.png", "Bidang utama untuk satu periode APBDes."),
            note("Ketelitian angka", "Jangan membuat dua entri dengan kombinasi tahun dan versi yang sama. Gunakan angka dari dokumen APBDes resmi dan jangan menghapus periode melalui panel.", "gold"),
        ]),
        ("Potensi dan Kelompok Tani", [
            p("Seluruh informasi pertanian dan peternakan dikelola dalam satu entri Potensi Pertanian agar datanya tidak tersebar ke banyak koleksi."),
            table(["Bagian", "Isi yang dikelola"], [["Narasi dan identitas", "Judul, subjudul, deskripsi, Gapoktan, serta narasi peternakan"], ["Galeri Peternakan", "Foto dan teks alternatif setiap foto"], ["Statistik Potensi", "Angka ringkas pertanian, peternakan, dan sarana pertanian"], ["Kelompok Tani", "Nama kelompok, ketua, dusun, komoditas, luas, dan anggota"], ["Produksi Pertanian", "Komoditas, luas, produktivitas, produksi, dan urutan"]]),
            steps("Buka Potensi Desa — Pertanian & Peternakan > Potensi Pertanian.", "Pilih bagian daftar yang ingin diperbarui.", "Tambahkan atau ubah item di dalam daftar tanpa membuat koleksi baru.", "Gunakan satuan yang konsisten dan cantumkan tahun serta sumber data.", "Terbitkan setelah seluruh bagian yang berubah diperiksa."),
        ]),
        ("UMKM Desa", [
        ...
        ("Galeri Foto", [
            ...
            bullets("Gunakan JPG, PNG, atau WebP.", "Setiap field gambar menolak berkas di atas 5 MB. Kompres gambar sebelum mengunggah jika ukurannya terlalu besar.", "CMS tidak mengompres gambar secara otomatis; gunakan nama berkas yang jelas dan tanpa karakter aneh.", "Pilih foto terang, tidak buram, dan tidak memperlihatkan dokumen pribadi warga."),
        ]),
        ("Beranda dan Identitas Desa", [
            p("Teks halaman tunggal dan identitas desa ditempatkan pada koleksi file agar hanya memiliki satu entri resmi."),
            steps("Buka Halaman — Beranda untuk judul utama, subjudul, gambar, sorotan, dan statistik.", "Buka Potensi Desa — Pertanian & Peternakan untuk narasi pertanian, Gapoktan, peternakan domba, dan galeri peternakan.", "Buka Layanan Desa — Pengantar untuk judul serta subjudul halaman layanan.", "Buka Sistem — Identitas Desa > Pengaturan Umum untuk alamat kantor, kontak, media sosial, peta, dan tahun APBDes aktif."),
            note("Dampak perubahan", "Perubahan Identitas Desa dapat muncul di beberapa halaman sekaligus, termasuk header, footer, Profil, dan Layanan.", "gold"),
        ]),
        ("Menyimpan dan Mengatasi Masalah", [
            ...
            h("Masalah umum"),
            bullets("Perubahan belum terlihat: tunggu beberapa menit lalu muat ulang halaman.", "Tidak bisa masuk: pastikan akun GitHub memiliki akses tulis ke repository Portal Rancajaya.", "Foto tidak muncul: unggah ulang JPG, PNG, atau WebP berukuran maksimal 5 MB dengan nama berkas sederhana.", "Website gagal diperbarui: catat informasi terakhir yang diubah dan hubungi PIC teknis.", "Salah isi: buka kembali data, perbaiki, lalu Terbitkan ulang.", "Menghapus entri: gunakan Hapus hanya pada Berita, Galeri, UMKM, Layanan Administrasi, atau Kontak; pastikan entri benar-benar tidak diperlukan."),
...
DEVELOPMENT = {
    ...
    "sections": [
        ...
        ("Stack dan Alasan Pemilihan", [
            table(["Komponen", "Peran dan alasan"], [["Astro 7", "Routing berbasis berkas dan keluaran statis; sesuai untuk portal informasi dengan JavaScript minimum"], ["Tailwind CSS 4 + Vite", "Token desain dan kelas utilitas melalui plugin Vite resmi Tailwind"], ["Content Layer + Zod 4", "Loader glob dan validasi struktur Markdown/JSON saat build sebelum perubahan tayang"], ["Decap CMS 3", "Antarmuka pengelolaan berbasis Git tanpa database CMS"], ["Chart.js 4", "Grafik APBDes dan demografi dari data yang sudah tersedia di halaman"], ["GitHub Actions + Pages", "Build dan hosting produksi gratis dalam satu riwayat repository"], ["Netlify", "OAuth proxy saja; bukan hosting produksi"]]),
        ...
        ("Alur Data dan Model Konten", [
            ...
            table(["Kelompok data", "Sumber", "Konsumen utama"], [["Berita", "src/content/berita/*.md", "Beranda dan halaman Informasi"], ["APBDes", "src/content/apbdes/*.md; satu berkas per tahun dan versi", "Profil dan grafik riwayat APBDes"], ["Profil dan struktur", "src/content/halaman/profil.json", "Halaman Profil"], ["Potensi pertanian", "src/content/halaman/pertanian.json", "Halaman Pertanian"], ["Dusun", "src/content/dusun/*.md", "Halaman Profil"], ["UMKM dan galeri", "src/content/umkm serta galeri", "Beranda, Pertanian, dan Informasi visual"], ["Layanan dan kontak", "layanan-administrasi serta kontak", "Beranda dan Layanan"], ["Identitas desa", "pengaturan/umum.json", "Navbar, Footer, peta, dan identitas desa"], ["Halaman tunggal", "halaman/*.json", "Teks khusus Beranda, Profil, Pertanian, dan Layanan"]]),
            table(["Status", "Perilaku"], [["release_status: Draf", "isPublishedContent mengembalikan false"], ["release_status: Terbit", "konten boleh ditampilkan"], ["aktif: false", "konten selalu disembunyikan"]]),
        ...
        ("Menjalankan Proyek Secara Lokal", [
            steps("Gunakan Node.js 22.12.0 atau lebih baru.", "Clone repository dan masuk ke folder portalrancajaya.github.io.", "Jalankan npm ci agar dependency mengikuti package-lock.json.", "Jalankan npm run dev.", "Buka http://localhost:4321/.", "Jalankan npm run build dan npm run preview sebelum commit."),
        ...
        ("Menambah atau Mengubah Collection", [
            steps("Definisikan atau ubah schema Zod di src/content.config.ts.", "Tambahkan loader glob dengan pattern dan base yang sesuai, lalu daftarkan collection pada export collections.", "Buat folder konten dan satu contoh berkas yang valid.", "Tambahkan collection serta field yang identik di public/admin/config.yml.", "Atur create, delete, identifier_field, summary, dan slug sesuai kebutuhan.", "Sesuaikan komponen atau halaman yang membaca data.", "Jalankan npm run build, lalu uji tambah dan edit melalui admin."),
        ...
        ("Konfigurasi CMS dan Publikasi", [
            ...
            table(["Operasi", "Collection"], [["Tambah + edit + hapus", "berita, galeri, UMKM, layanan administrasi, kontak"], ["Tambah + edit; delete false", "APBDes per periode"], ["Edit saja", "beranda, profil dan struktur organisasi, dusun, pertanian dan peternakan, pengantar layanan, identitas desa"]]),
        ...
        ("Deployment dan CI/CD", [
            ...
            code("checkout -> configure-pages -> setup-node 22 -> npm ci -> npm run build\n-> upload-pages-artifact ./dist -> deploy-pages"),
